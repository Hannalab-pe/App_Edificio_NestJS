...
import { CreateTipoCronogramaDto, UpdateTipoCronogramaDto } from '../../../dtos';
...
export class TipoCronogramaService implements ITipoCronogramaService {
    ...
    async create(createTipoCronogramaDto: CreateTipoCronogramaDto): Promise<BaseResponseDto<TipoCronograma>> {
        try {
            // Verificar si ya existe un tipo de cronograma con el mismo tipo
            const tipoCronogramaExistente = await this.tipoCronogramaRepository.findOne({
            ...
            const nuevoTipoCronograma = this.tipoCronogramaRepository.create(createTipoCronogramaDto);
            const tipoCronogramaGuardado = await this.tipoCronogramaRepository.save(nuevoTipoCronograma);
    ...
    async findAll(): Promise<BaseResponseDto<TipoCronograma[]>> {
        try {
            ...
            return {
                success: true,
                message: tiposCronograma.length > 0 ? 'Tipos de cronograma encontrados' : 'No se encontraron tipos de cronograma',
        ...
        try {
            ...
            if (!tipoCronograma) {
                return {
                    ...
                    data: null,
                    error: { message: 'No existe un tipo de cronograma con el ID proporcionado' }
                };
            }
    ...
    async update(id: string, updateTipoCronogramaDto: UpdateTipoCronogramaDto): Promise<BaseResponseDto<TipoCronograma>> {
        try {
            const tipoCronograma = await this.tipoCronogramaRepository.findOne({
                where: { idTipoCronograma: id }
            });
            ...
            if (updateTipoCronogramaDto.tipoCronograma &&
            ...
            const tipoCronogramaActualizado = await this.tipoCronogramaRepository.save({
                ...tipoCronograma,
    ...
    async remove(id: string): Promise<BaseResponseDto<undefined>> {
        try {
            ...
            if (tipoCronograma.cronogramas && tipoCronograma.cronogramas.length > 0) {
                return {
                    success: false,
                    message: 'No se puede eliminar el tipo de cronograma porque tiene cronogramas asociados',
                    data: undefined,
    ...
    async findByTipo(tipoCronograma: string): Promise<BaseResponseDto<TipoCronograma>> {
        try {
            const tipoCronogramaEncontrado = await this.tipoCronogramaRepository.findOne({
            ...
            if (!tipoCronogramaEncontrado) {
                return {
                    ...
                    message: 'Tipo de cronograma no encontrado',
                    data: null,
                    error: { message: `No existe un tipo de cronograma con el tipo ${tipoCronograma}` }
                };
            }
