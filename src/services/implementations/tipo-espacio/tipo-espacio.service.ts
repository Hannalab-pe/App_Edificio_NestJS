...
export class TipoEspacioService implements ITipoEspacioService {
    ...
    constructor(@InjectRepository(TipoEspacio) private readonly tipoEspacioRepository: Repository<TipoEspacio>) { }

    async create(createTipoEspacioDto: CreateTipoEspacioDto): Promise<BaseResponseDto<TipoEspacio>> {
        ...
        try {
            ...
            const tipoEspacioGuardado = await this.tipoEspacioRepository.save(tipoEspacio);
            return {
                success: true,
    ...
    async findOne(id: string): Promise<BaseResponseDto<TipoEspacio>> {
        ...
        try {
            const tipoEspacio = await this.tipoEspacioRepository.findOne({ where: { idTipoEspacio: id } });
            if (!tipoEspacio) {
                return {
                    success: false,
    ...
    async update(id: string, updateTipoEspacioDto: UpdateTipoEspacioDto): Promise<BaseResponseDto<TipoEspacio>> {
        ...
        try {
            ...
            const tipoEspacioActualizado = await this.tipoEspacioRepository.save(tipoEspacio);
            return {
                success: true,
    ...
    async findByNombre(nombre: string): Promise<BaseResponseDto<TipoEspacio>> {
        ...
        try {
            const tipoEspacio = await this.tipoEspacioRepository.findOne({ where: { nombre } });
            if (!tipoEspacio) {
                return {
                    success: false,
            ...
            return {
                ...
                data: null,
                error: {
                    message: 'Error al buscar el tipo de espacio por nombre: ' + error.message,
                    statusCode: 400
                }
            };
        }
