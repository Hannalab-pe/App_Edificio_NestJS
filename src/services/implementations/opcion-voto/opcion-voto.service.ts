...
export class OpcionVotoService implements IOpcionVotoService {
  ...
  async create(
    ...
    try {
      ...
      const opcionExistente = await this.opcionVotoRepository
        ...
        .andWhere('opcion.idVotacion = :votacionId', {
          votacionId: createOpcionVotoDto.idVotacion,
        })
        .getOne();
      ...
      const ordenExistente = await this.opcionVotoRepository
        .createQueryBuilder('opcion')
        .where('opcion.ordenPresentacion = :orden', {
          orden: createOpcionVotoDto.ordenPresentacion,
        })
        .andWhere('opcion.idVotacion = :votacionId', {
          votacionId: createOpcionVotoDto.idVotacion,
        })
      ...
      if (!opcionCompleta) {
        throw new BadRequestException(
          'Error al recuperar la opción de voto creada',
        );
      }
      ...
      throw new BadRequestException(
        `Error al crear la opción de voto: ${error.message}`,
      );
    }
  }
  ...
  async findAll(): Promise<GetOpcionesVotoResponseDto> {
    try {
      ...
      throw new BadRequestException(
        `Error al obtener las opciones de voto: ${error.message}`,
      );
    }
  }
  ...
  async findOne(id: string): Promise<GetOpcionVotoResponseDto> {
    try {
      ...
      if (!opcion) {
        throw new NotFoundException(
          `Opción de voto con ID ${id} no encontrada`,
        );
      }
      ...
      throw new BadRequestException(
        `Error al obtener la opción de voto: ${error.message}`,
      );
    }
  }
  ...
  async update(
    ...
    try {
      ...
      // Validar que la votación no esté finalizada
      if (
        opcion.idVotacion.estado === 'FINALIZADA' ||
        opcion.idVotacion.estado === 'CANCELADA'
      ) {
        throw new BadRequestException(
      ...
      if (opcion.votos && opcion.votos.length > 0) {
        if (
          updateOpcionVotoDto.opcion &&
          updateOpcionVotoDto.opcion !== opcion.opcion
        ) {
          throw new BadRequestException(
      ...
      if (
        ...
        const opcionExistente = await this.opcionVotoRepository
          .createQueryBuilder('opcion')
          .where('opcion.opcion = :texto', {
            texto: updateOpcionVotoDto.opcion,
          })
          .andWhere('opcion.idVotacion = :votacionId', {
            votacionId: opcion.idVotacion.idVotacion,
          })
          .getOne();
      ...
      if (
        updateOpcionVotoDto.ordenPresentacion &&
        updateOpcionVotoDto.ordenPresentacion !== opcion.ordenPresentacion
      ) {
        const ordenExistente = await this.opcionVotoRepository
          .createQueryBuilder('opcion')
          .where('opcion.ordenPresentacion = :orden', {
            orden: updateOpcionVotoDto.ordenPresentacion,
          })
          .andWhere('opcion.idVotacion = :votacionId', {
            votacionId: opcion.idVotacion.idVotacion,
          })
      ...
      if (!opcionCompleta) {
        throw new BadRequestException(
          'Error al recuperar la opción de voto actualizada',
        );
      }
      ...
      throw new BadRequestException(
        `Error al actualizar la opción de voto: ${error.message}`,
      );
    }
  }
  ...
  async remove(id: string): Promise<DeleteOpcionVotoResponseDto> {
    try {
      ...
      throw new BadRequestException(
        `Error al eliminar la opción de voto: ${error.message}`,
      );
    }
  }
  ...
  async findByVotacion(
    ...
    try {
      ...
      throw new BadRequestException(
        `Error al obtener las opciones de la votación: ${error.message}`,
      );
    }
  }
}
