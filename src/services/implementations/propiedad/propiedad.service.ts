...
export class PropiedadService implements IPropiedadService {
    ...
    async create(createPropiedadDto: CreatePropiedadDto): Promise<BaseResponseDto<Propiedad>> {
        try {
            ...
            const nuevaPropiedad = this.propiedadRepository.create({
                ...createPropiedadDto,
                areaM2: createPropiedadDto.areaM2 ? createPropiedadDto.areaM2.toString() : null,
                valorComercial: createPropiedadDto.valorComercial ? createPropiedadDto.valorComercial.toString() : null,
                estaActivo: createPropiedadDto.estaActivo ?? true
            });

            const propiedadGuardada = await this.propiedadRepository.save(nuevaPropiedad);
    ...
    async update(id: string, updatePropiedadDto: UpdatePropiedadDto): Promise<BaseResponseDto<Propiedad>> {
        try {
            const propiedad = await this.propiedadRepository.findOne({
                where: {
                    idPropiedad: id,
            ...
            if (updatePropiedadDto.numeroDepartamento &&
            ...
            const datosActualizacion = {
                ...updatePropiedadDto,
                areaM2: updatePropiedadDto.areaM2 !== undefined ?
                    (updatePropiedadDto.areaM2 ? updatePropiedadDto.areaM2.toString() : null) :
                    propiedad.areaM2,
                valorComercial: updatePropiedadDto.valorComercial !== undefined ?
                    (updatePropiedadDto.valorComercial ? updatePropiedadDto.valorComercial.toString() : null) :
    ...
    async findByTipoPropiedad(tipoPropiedad: string): Promise<BaseResponseDto<Propiedad[]>> {
        try {
            const propiedades = await this.propiedadRepository.find({
                where: {
    ...
    async findByEstadoOcupacion(estadoOcupacion: string): Promise<BaseResponseDto<Propiedad[]>> {
        try {
            const propiedades = await this.propiedadRepository.find({
                where: {
    ...
    async findByNumeroDepartamento(numeroDepartamento: string): Promise<BaseResponseDto<Propiedad>> {
        try {
            const propiedad = await this.propiedadRepository.findOne({
                where: {
            ...
            if (!propiedad) {
                return {
                    ...
                    message: 'Propiedad no encontrada',
                    data: null,
                    error: { message: `No existe una propiedad con el número ${numeroDepartamento}` }
                };
            }
