...
export class PresupuestoService implements IPresupuestoService {
  ...
  ): Promise<BaseResponseDto<Presupuesto>> {
    try {
      ...
      const nuevoPresupuesto =
        this.presupuestoRepository.create(createPresupuestoDto);
      const presupuestoGuardado =
        await this.presupuestoRepository.save(nuevoPresupuesto);
  ...
  async update(
    ...
    try {
      ...
      if (
        updatePresupuestoDto.anio ||
        updatePresupuestoDto.mes ||
        updatePresupuestoDto.concepto
      ) {
        ...
        const concepto =
          updatePresupuestoDto.concepto ?? presupuestoExistente.concepto;
        ...
        if (duplicado && duplicado.idPresupuesto !== id) {
          return {
            success: false,
            message:
              'Ya existe un presupuesto con el mismo año, mes y concepto',
            data: null,
            error: {
              statusCode: 409,
              message: 'Conflicto de datos únicos',
