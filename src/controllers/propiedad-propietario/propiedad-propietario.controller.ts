...
export class PropiedadPropietarioController {
  ...
  @ApiOperation({
    ...
    description:
      'Asocia un propietario a una propiedad que ya debe estar registrada en el edificio. Si el propietario no existe, lo crea automáticamente.',
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiResponse({
    status: 400,
    description:
      'Error en los datos enviados, la propiedad no existe, o la relación ya existe',
  })
  async create(
    @Body() createPropiedadPropietarioDto: CreatePropiedadPropietarioDto,
  ): Promise<BaseResponseDto<PropiedadPropietario>> {
    return await this.propiedadPropietarioService.create(
